# This script creates key-value pairs and files for research-questions.html
...
# Get the total responses (i.e. a row count) from the prepared OSF file.
keyValuePairs[["TOTAL_RESPONDENTS"]] = aiDaily %>% filter(!is.na(Support)) %>% nrow()

# Write the final list to a JSON file
write_json(keyValuePairs, paste0(baseDirectory, "json/", "research-questions.json"))

print(paste("create-research-questions-dictionary.R wrote to", paste0(baseDirectory, "json/", "research-questions.json") ))
